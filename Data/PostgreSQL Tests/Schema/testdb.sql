...
CREATE OR REPLACE FUNCTION myfunc() RETURNS REFCURSOR
LANGUAGE plpgsql
AS $$
DECLARE
	my_refcur REFCURSOR = 'nlis_refcur1';
BEGIN
	Open my_refcur for
...
CREATE OR REPLACE FUNCTION myfunc_err() RETURNS REFCURSOR
LANGUAGE plpgsql
AS $$
DECLARE
	my_refcur REFCURSOR = 'P_OUT_CURSOR';
BEGIN
	Open my_refcur for
		SELECT product_id, product_name
		FROM products WHERE product_id < 5;
...
CREATE OR REPLACE FUNCTION myfunc2() RETURNS SETOF REFCURSOR
LANGUAGE plpgsql
AS $$
DECLARE
	my_refcur1 REFCURSOR = 'nlis_refcur1';
	my_refcur2 REFCURSOR = 'nlis_refcur2';
BEGIN
	Open my_refcur1 for
		SELECT product_id, product_name
		FROM products WHERE product_id < 5;
	return next my_refcur1;
	Open my_refcur2 for
		SELECT product_id, product_name
		FROM products WHERE product_id >= 5 AND product_id < 10;
	return next my_refcur2;
END; $$;

CREATE OR REPLACE FUNCTION myfunc3(p_from int, p_to int) RETURNS SETOF REFCURSOR
LANGUAGE plpgsql
AS $$
DECLARE
	my_refcur1 REFCURSOR = 'nlis_refcur1';
BEGIN
	Open my_refcur1 for
		SELECT product_id, product_name
		FROM products WHERE product_id >= p_from AND product_id <= p_to;
	return next my_refcur1;
END; $$;

CREATE OR REPLACE FUNCTION myfunc4(p_name VARCHAR(30)) RETURNS SETOF REFCURSOR
LANGUAGE plpgsql
AS $$
DECLARE
	my_refcur1 REFCURSOR = 'nlis_refcur1';
BEGIN
	Open my_refcur1 for
		SELECT product_id, product_name
		FROM products WHERE product_name like p_name;
	return next my_refcur1;
END; $$;

CREATE OR REPLACE FUNCTION myfunc5(result out int)
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE myproc(out out_refcur REFCURSOR)
LANGUAGE plpgsql
AS $$
BEGIN
	out_refcur = 'nlis_refcur1';
	Open out_refcur for
		SELECT product_id, product_name
		FROM products WHERE product_id < 5;
END; $$;

CREATE OR REPLACE PROCEDURE myproc3(p_from int, p_to int, out out_refcur REFCURSOR)
LANGUAGE plpgsql
AS $$
BEGIN
	out_refcur = 'nlis_refcur1';
